...
ArmControl::ArmControl()
	: MinorMode(),
	  ...
	  dropTime(dropTime),
	  propGain(propGain),
	  holdPropGain(holdPropGain),
	  intGain(intGain),
	  derivGain(derivGain),
	  baseRestPosition(baseRestPosition),
	  midRestPosition(midRestPosition),
	  baseHoldPosition(baseHoldPosition),
	  midHoldPosition(midHoldPosition),
	  stepperMicrosDelay(stepperMicrosDelay),
	  numPulses(numPulses),
	  initialAdjMidTarget(initialAdjMidTarget),
	  initialAdjBaseTarget(initialAdjBaseTarget),
	  finalAdjMidTarget(finalAdjMidTarget),
	  finalAdjBaseTarget(finalAdjBaseTarget),
	  
	  //pin constants
	  baseAnglePin(baseAnglePin),
	  baseMotorNumber(baseMotorNumber),
	  babyMotorNum(babyMotorNum),
	  catchSwitch(catchSwitch),
	  noCatchSwitch(noCatchSwitch),
	  innerClawSwitch(innerClawSwitch),
	  stepperDirPin(stepperDirPin),
	  stepperPulsePin(stepperPulsePin)
